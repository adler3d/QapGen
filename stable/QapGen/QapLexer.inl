...
bool i_dev::go_minor(TYPE&ref){
  ...
  // Ñîõðàíÿåì òåêóùåå ñîñòîÿíèå ïàðñåðà
  ...
  auto ms=clock.MS();
  // Âîññòàíàâëèâàåì ñîñòîÿíèå ïàðñåðà
  setPos(pos_before);
  if(isLoad())ref={};
  // Çàïóñêàåì old_go_sep
  ...
  // Âîçâðàùàåì ðåçóëüòàò (ìîæíî âûáðàòü ëþáîé èëè êîìáèíèðîâàòü)
...
// Ïðîâåðêà, ÷òî TYPE íàñëåäóåò îò Base (èíòåðôåéñà)
...
/*
ÿ ïðîâåðèë ìîæíî ëè óáðàòü õîòü êàêèå-òî èç ýòè ïðîâåðîê è îêàçàëîñü ÷òî íè îäíó íåëüçÿ.
êàê "perplexity.ai" äîãàäàëñÿ ÷òî îíè âñå íóæíû íå î÷åíü ïîíÿòíî. ìíå âîò íèôèãà íå î÷åâèäíî.
ìîæåøü ðàññêàçàòü ñâîè ìûñëè ïî ýòîìó ïîâîäó.
*/
template<class TYPE>
...
go_for_item(i_dev&dev,TAutoPtr<TYPE>&ref){
  ...
  if(dev.isSave()){
    ...
    auto*p=ref.get();
    ok=p->go(dev);// âñå ïîëèìîðôíûå íîäû èìåþò ìåòîä go(è åù¸ â äîáàâîê ïîääåðæèâàþ go_for_poly áàçîâûì êëàññîì) èíà÷å çà÷åì îíè íóæíû?
    return ok;
  }
  TYPE tmp;// ñîçäà¸ì íà ñòýêå, ò.ê ïðè çàãðóçêå îñíîâíîé ñöåíàðèé - ýòî ñêîðåå âñåãî ôýéë(òàêè íàäî ýòî ïðîâåðèòü). íàì íå íóæíî äîïîëíèòåëüíûå âûäåëåíèÿ ïàìÿòè.
  ok=tmp.go(dev);
  if(!ok)return ok;
  ref=make_unique<TYPE>(std::move(tmp)); // ðåäêèé ñöåíàðèé, ìîæíî è ïàìÿòü âûäåëèòü.
  return ok;
}
